"""Parallel, resumable Stage 2 extraction for the Codex orchestrator output.

This follows prompts/orchestrator_codex.md for Stage 2, with two execution
concessions that do not change the output schema:
- resume from existing s02_extracted.jsonl records;
- extract each exact SHA-256 duplicate once, then write all duplicate rows.
"""
...
UNIQUE_SCRATCH = OUTPUT_FOLDER / "scratch" / "s02_unique"
...
def extract_one(rec: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        if rec["file_type"] == "pdf":
            ...
            with pdfplumber.open(rec["original_path"]) as pdf:
        ...
        result_path = UNIQUE_SCRATCH / f"{rec['doc_id']}.json"
        tmp_path = result_path.with_suffix(".json.tmp")
        with tmp_path.open("w", encoding="utf-8", errors="replace") as f:
            f.write(json.dumps({k: v for k, v in result.items() if not k.startswith("_")}, ensure_ascii=False))
        os.replace(tmp_path, result_path)
        return {"doc_id": rec["doc_id"], "result_path": str(result_path), "_error": str(exc)}
    ...
    with tmp_path.open("w", encoding="utf-8", errors="replace") as f:
        f.write(json.dumps({k: v for k, v in result.items() if not k.startswith("_")}, ensure_ascii=False))
    os.replace(tmp_path, result_path)
    return {"doc_id": rec["doc_id"], "result_path": str(result_path), "_error": None}
...
def main() -> int:
    workers = int(os.environ.get("STAGE2_WORKERS", "6"))
    inventory = load_jsonl(INVENTORY_PATH)
    if not inventory:
        print(f"Missing or empty inventory: {INVENTORY_PATH}", file=sys.stderr)
        return 1
    ...
    UNIQUE_SCRATCH.mkdir(parents=True, exist_ok=True)
    start = time.time()
    written = 0
    failures = 0
            ...
            json.dumps(
                {
                    ...
                    "duplicate_records_filled": written,
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    with open(result_ref["result_path"], "r", encoding="utf-8", errors="replace") as f:
                        base = json.load(f)
                    base["_error"] = result_ref.get("_error")
                except Exception as exc:
                    base = {
